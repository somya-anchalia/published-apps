...
class Utilities:
    ...
    def check_collection_exists(self, collection_name, do_create=False, **kwargs):
        ...
        try:
            ...
            entry = json.loads(sc)["entry"]
            self._log.info("action=check_collection_exists do_create={} sc={}".format(do_create, json.dumps(entry)))
            ...
            if do_create:
                ...
                self._log.info(
                    "action=check_collection_exists do_create={} response={}".format(do_create, json.dumps(sc)))
                self.check_transform_exists(t_name, do_create=do_create, collection=c_name, external_type="kvstore")
                ret_obj = json.loads(sc)["entry"]
                if "fields_list" in kwargs:
                    fl = kwargs["fields_list"]
                    fl.append("_key")
                    fl_s = ", ".join(sorted(fl))
    ...
    def check_transform_exists(self, transform_name, do_create=False, **kwargs):
        ...
        try:
            ...
            if do_create:
                ...
                self._log.info("action=check_transform_exists do_create={} response={}".format(do_create, json.dumps(sc)))
                return json.loads(sc)["entry"]
            else:
                return None
    ...
    def get_configuration(self, filename, stanza):
        try:
            ...
            self._log.error('message="{}" exception_type="{}" '
                            'exception_arguments="{}" filename="{}" exception_line="{}" '.format(str(e), type(e).__name__, e, fname, exc_tb.tb_lineno))
            return {}
                ...
                try:
                    ...
                    self._log.debug(
                        "action=get_credential msg=found_clear_password realm={} cuser={} ".format(realm,
                                                                                                   cuser))
                    # Account for urllib library changes in Python 3.x
                    return urllib.parse.unquote(clear_pass)
    ...
    def is_cloud(self):
        try:
            ...
            sr, sc = self._make_get_request(uri, args={"output_mode": "json"})
            instance_type = json.loads(sc)['entry'][0]['content']['instance_type']
